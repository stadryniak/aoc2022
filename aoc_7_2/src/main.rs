use std::{
    ...
    path::Path,
    str::FromStr,
    string::ParseError, collections::HashMap,
};
...
impl Area {
    ...
    fn dir_sizes(&self) -> HashMap<&Dir, usize> {
        let mut dir_size: HashMap<&Dir, usize> = HashMap::new();
        for dir in &self.tree {
            dir_size.insert(dir, self.dir_size(dir));
        }
        dir_size
    }
}
...
fn main() {
    ...
    let mut sorted: Vec<_> = dir_sizes.iter().collect();
    sorted.sort_by(|a, b| a.1.cmp(b.1));
    let to_free = sorted.last().unwrap().1 - (70000000 - 30000000);
    
    for dir_size in sorted {
        if &to_free < dir_size.1 {
            println!("{}", dir_size.1);
            break;
        }
    }
