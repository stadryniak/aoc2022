...
fn calculate_response(opponent_move: &RSP, expected_result: &str) -> RSP {
    match expected_result {
        "X" => {
            for mov in RSP::iter() {
                if opponent_move > &mov {
                    return mov
                }
            }
            panic!("Invalid expected result: {}", expected_result);
        },
        "Y" => *opponent_move,
        "Z" => {
            for mov in RSP::iter() {
        ...
        _ => panic!("Invalid expected result: {}", expected_result),
    }
}
...
fn calculate_game_points(str: &String) -> i32 {
    ...
    let me = calculate_response(&opponent, words[1]);
